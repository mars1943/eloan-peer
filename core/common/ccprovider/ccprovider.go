...
func CheckInsantiationPolicy(name, version string, cdLedger *ChaincodeData) error {
	...
	if err != nil {
		//rongzer,王剑修改,其它节点可以运行帐本上的chaincode
		if cdLedger.Buf != nil {
			ccpack, _ := GetCCPackage(cdLedger.Buf)
			ccdata = ccpack.GetChaincodeData()
		}
		if ccdata == nil {
			return fmt.Errorf("Chaincode data for cc %s/%s was not found", name, version)
		}
	}
...
type ChaincodeData struct {
	...
	//rongzer,王剑增加，保存buf for the chaincode
	Buf []byte `protobuf:"bytes,9,opt,name=codepage,proto3"`

	TxID string `protobuf:"bytes,10,opt,name=desc"`

	Timestamp *google_protobuf.Timestamp `protobuf:"bytes,11,opt,name=timestamp" json:"timestamp,omitempty"`

	Desc string `protobuf:"bytes,12,opt,name=desc"`
	//rongzer,增加结束
}

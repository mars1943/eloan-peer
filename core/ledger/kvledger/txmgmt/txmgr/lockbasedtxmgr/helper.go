...
type mapRLists struct {
	m map[string]*statedb.RList
}
...
func (h *queryHelper) getState(ns string, key string) ([]byte, error) {
	...
	if strings.HasPrefix(key, "__RLIST_") {
		lisParamStr := key[8:]
		lisMethod := lisParamStr[:3]
		lisParam := strings.Split(lisParamStr[4:], ",")
		if h.rLists == nil {
			h.rLists = make(map[string]*mapRLists)
		}
		maprLists := h.rLists[ns]
		if maprLists == nil {
			maprLists = &mapRLists{make(map[string]*statedb.RList)}
			h.rLists[ns] = maprLists
		}

		rList := maprLists.m[lisParam[0]]
		if rList == nil {
			rList = statedb.NewRList(h.txmgr.db, ns, lisParam[0])
			maprLists.m[lisParam[0]] = rList
		}

		val := []byte("")
		if strings.EqualFold(lisMethod, "LEN") {
			val = []byte(strconv.Itoa(rList.Size))
		} else if strings.EqualFold(lisMethod, "GET") {
			b, err := strconv.Atoi(lisParam[1])
			if err != nil {
				b = 0
			}
			val = []byte(rList.GetIdByIndex(b))
		} else if strings.EqualFold(lisMethod, "IDX") {
			val = []byte(strconv.Itoa(rList.IndexOf(lisParam[1])))
		}
		return val, nil
	}

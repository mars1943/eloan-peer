...
//rongzer,songxiang
func (vdb *VersionedDB) CommitThread(re_results chan<- int, ns string, k string, vv *statedb.VersionedValue, flag int) error {
...
func (vdb *VersionedDB) ApplyUpdates(batch *statedb.UpdateBatch, height *version.Height) error {
	...
	for _, ns := range namespaces {
		...
		for k, vv := range updates {
			go vdb.CommitThread(results, ns, k, vv, flag)
		}
		flag++
	}
	// Record a savepoint at a given height
	// rongzer,songxiang 此处代码整合成了CommitThread函数
	err := vdb.recordSavepoint(height)
	//logger.Infof("~~~~end recordSavepoint~~~~")
	if err != nil {

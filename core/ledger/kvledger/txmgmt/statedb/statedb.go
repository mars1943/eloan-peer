...
type UpdateBatch struct {
	...
	cals      map[string]*nsUpdates
	rLists    map[string]*nsRLists
	UpdateNum int
	CalNum    int
	RListNum  int
}
...
func NewUpdateBatch() *UpdateBatch {
	return &UpdateBatch{updates: make(map[string]*nsUpdates), cals: make(map[string]*nsUpdates), rLists: make(map[string]*nsRLists)}
}
...
// GetUpdatedNamespaces returns the names of the namespaces that are updated
func (batch *UpdateBatch) GetCaledNamespaces() []string {
	namespaces := make([]string, len(batch.cals))
	i := 0
	for ns := range batch.cals {
		namespaces[i] = ns
		i++
	}
	return namespaces
...
// GetUpdatedNamespaces returns the names of the namespaces that are updated
func (batch *UpdateBatch) GetRListsNamespaces() []string {
	namespaces := make([]string, len(batch.rLists))
	i := 0
	for ns := range batch.rLists {
		namespaces[i] = ns
		i++
	}
	return namespaces
...
//rongzer GetUpdateSize,取更新
func (batch *UpdateBatch) GetUpdateSize() int {
	count := 0
	for ns := range batch.updates {
...
// GetUpdates returns all the updates for a namespace
func (batch *UpdateBatch) GetCals(ns string) map[string]*VersionedValue {
	nsUpdates, ok := batch.cals[ns]
	if !ok {
		return nil
	}
	return nsUpdates.m
...
// GetUpdates returns all the updates for a namespace
func (batch *UpdateBatch) GetRLists(ns string) map[string]*RList {
	nsRLists, ok := batch.rLists[ns]
	if !ok {
		return nil
	}
	return nsRLists.m
...
func (batch *UpdateBatch) GetOrCreateNsCals(ns string) *nsUpdates {
	nsCals := batch.cals[ns]
	if nsCals == nil {
		nsCals = newNsUpdates()
		batch.cals[ns] = nsCals
	}
	return nsCals
}

func (batch *UpdateBatch) GetOrCreateNsRLists(ns string) *nsRLists {
	mapRLists := batch.rLists[ns]
	if mapRLists == nil {
		mapRLists = &nsRLists{make(map[string]*RList)}
		batch.rLists[ns] = mapRLists
	}
	return mapRLists
}

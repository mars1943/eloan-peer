...
func (lscc *LifeCycleSysCC) getCCCode(ccname string, cdbytes []byte) (*ccprovider.ChaincodeData, *pb.ChaincodeDeploymentSpec, []byte, error) {
	...
	//rongzer,王剑将ccpack的buf保存到state，为了其它节点用此合约
	buf := cd.Buf
...
func (lscc *LifeCycleSysCC) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case "updateToVersion":
		...
		if len(args) == 5 {
			bPayload, err := base64.StdEncoding.DecodeString(string(args[4]))
			if err != nil {
				return shim.Error(err.Error())
			}
		...
		if err != nil {
			logger.Errorf("error update chaincode %s to version %s on channel: %s", ccname, ccversion, err)
			return shim.Error(err.Error())
		}
	...
	case "getccdatahis":
		...
		ccbytess, _ := proto.Marshal(input)
	...
	case "getvccdata":
		...
		for keysIter.HasNext() {
			...
			if cd.Version == ccversion {
				return shim.Success(response.Value)
			}
		}

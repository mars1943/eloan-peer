...
type ChaincodeStubInterface interface {
	...
	//rongzer,RList的相关方法
	GetChannelHeader() (*cb.ChannelHeader, error)
	Add(rlistName string, id string)
	AddByIndex(rlistName string, nIndex int, id string)
	Remove(rlistName string, id string)
	Size(rlistName string) int
	Get(rlistName string, nIndex int) string
	IndexOf(rlistName string, id string) int

	QueryStateHistory(idKey string) []byte
}
